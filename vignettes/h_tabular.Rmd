...
There are two possible special values in the `.model` and `.state` columns:  'discount' and
'count_type'.  The first provides an alternate way to specify the discount rate for each value;
including both a 'discount' row and '.discount' columns will cause an error.  The second allows
the user to specify which type of counts should be used to calculate each value.   The possible
values are 'beginning', 'end', and 'life-table', or the name of a function to be used
to modify counts; see [correct_counts()].  If no
'count_type' column is included, all values will be calculated using the 'method' specified when
running a model.  Below is an alternate version of the state file above, using a discount row, 
and specifying that cost be calculated based on counts at the beginning of the cycle (since
transitions happen at the end), while qalys should be calculated based on the average of
count at the beginning and end.

```{r echo = FALSE}
heemod:::read_file(system.file("tabular/thr/THR_states_discount_and_count_type.csv", 
                               package = "heemod")) %>% 
  format_na %>% 
  knitr::kable()
```
